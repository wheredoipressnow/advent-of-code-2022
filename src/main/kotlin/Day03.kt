private val cost = ('a'..'z') + ('A'..'Z')

fun main() {
    println(prioritizeItemRearrangement(TEST3_INPUT))
    println(prioritizeItemRearrangement(PUZZLE3_INPUT))

    println(prioritizeItemExtendedGroup(TEST3_INPUT))
    println(prioritizeItemExtendedGroup(PUZZLE3_INPUT))
}

private fun prioritizeItemRearrangement(rucksacks: String) = rucksacks
    .split("\n")
    .map { it.chunked(it.length / 2) }
    .flatMap { it.first().toSet() intersect it.last().toSet() }
    .sumOf { cost.indexOf(it) + 1 }

private fun prioritizeItemExtendedGroup(rucksacks: String) = rucksacks
    .split("\n")
    .chunked(3)
...
private const val PUZZLE3_INPUT = """NJvhJcQWTJWTNTFFMTqqGqfTmB
